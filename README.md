...
Whatever you've written before, three things about Go tend to surprise people:
there are **no classes**, **no inheritance**, and **no exceptions**. None of
those are missing features — Go replaces each with something simpler, and the
guides below map it to what you already know.

Pick the one that matches your background:

- [Coming from C#](./docs/coming-from-csharp.md)
- [Coming from Python](./docs/coming-from-python.md)
- [Coming from Java](./docs/coming-from-java.md)

Each is a ten-minute read with side-by-side code, and you only need the one.
Coming from something else entirely? Read whichever is closest — the Go half of
every comparison is the same.
